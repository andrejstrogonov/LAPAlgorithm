...
class OptimizedExpertSystem:
    ...
    def backward_inference(self, goal, path=None, depth=0, max_depth=100):
        ...
        if path is None:
            ...
            self.best_rule_paths = {}  # Track the best path to each goal
        ...
        for rule in relevant_rules:
            ...
            for premise in rule.premises:
                if not self.backward_inference(premise, path, depth + 1, max_depth):
                    ...
                    break
                # Add rules used for this premise
                if premise in self.best_rule_paths:
                    current_path_rules.extend(self.best_rule_paths[premise])

            if all_premises_true:
                # Add this rule to the current path
                current_path_rules.append(rule)

                # Check if this path uses fewer rules than our current best
                if len(current_path_rules) < min_rule_count:
                    min_rule_count = len(current_path_rules)
                    best_rule = rule
                    self.best_rule_paths[goal] = current_path_rules

        # If we found a valid path
        if best_rule is not None:
            # Update the used_rules list with unique rules from the best path
            for rule in self.best_rule_paths[goal]:
