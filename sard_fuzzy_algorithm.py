...
def fuzzy_fitness(chromosome, target):
    score = 0
    ...
    elif score > 0.7 * len(target):
        return 0.8  # Good match
    elif score > 0.5 * len(target):
        return 0.6  # Average match
    else:
...
def solve_sard_fuzzy(target, pop_size=100, max_gens=100, crossover_rate=0.8, mutation_rate=0.1):
    gene_length = len(target)
    population = generate_initial_population(pop_size, gene_length)
    for gen in range(max_gens):
        fitnesses = [fuzzy_fitness(chromosome, target) for chromosome in population]
        best_chromosome = max(zip(fitnesses, population), key=lambda x: x[0])[1]
        print(f"Generation {gen}: Best fitness = {fuzzy_fitness(best_chromosome, target)}")
    ...
    best_solution = max(zip([fuzzy_fitness(chromosome, target) for chromosome in population], population), key=lambda x: x[0])[1]
    return best_solution

# Example usage
target = [1, 0, 1, 1, 0, 0, 1, 0]
solution = solve_sard_fuzzy(target)
